...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    // STRICT DUAL CNIC IMAGE ATTACHMENT VALIDATION (FRONT & BACK)
    const frontImg = cnic_front_file_name || cnic_file_name;
    const backImg = cnic_back_file_name || cnic_file_name;
    if (!frontImg || frontImg.toString().trim() === '' || !backImg || backImg.toString().trim() === '') {
      return res.status(400).json({ success: false, error: 'Both CNIC Front and Back images are STRICTLY MANDATORY for farmer registration.' });
    }
